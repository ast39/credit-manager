                    ...
                    <div class="card-body bg-white">
                        ...
                        @elsedesktop
                            <table class="table table-borderless">
                                @forelse($credits as $credit)
                                    <tr>
                                        <td colspan="3" class="text-start">{!! Icons::get(Icons::TITLE) !!} <a class="text-decoration-none text-primary" href="{{ route('credit.check.show', $credit->calc_id) }}">{{ $credit->title ?? '' }}</a></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr class="border-bottom">
                                        ...
                                        <td class="text-end">
                                            <form method="post" action="{{ route('credit.calc.destroy', $credit->calc_id) }}">
                                                @csrf
                                                @method('DELETE')

                                                <div class="mmot-table__action">
                                                    <a title="Открыть" href="{{ route('credit.calc.show', $credit->calc_id) }}" class="mmot-table__action__one"><svg class="mmot-table_view mmot-table__ico"><use xlink:href="#site-view"></use></svg></a>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="text-center">
                                            <div class="text-center p-2 mb-2 mt-2 bg-light bg-gradient text-success rounded">{{ __('У вас нет проверок в истории') }}</div>
                                        </td>
                                    </tr>
                                @endforelse
                            </table>
